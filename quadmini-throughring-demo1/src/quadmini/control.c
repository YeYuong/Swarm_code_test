...
#define sign_s(s)           (s < 0 ? -1 : (s > 0 ? 1 : 0 ))//需要一个变量来接一下
...
void ctrl_reset(struct ctrl_data_ty * ctrl_data)
{
	...
	ctrl_data->x_posi_ctrl.expect_x_posi_d = 0;
	ctrl_data->x_posi_ctrl.expect_x_posi_old = 0;
	ctrl_data->x_posi_ctrl.expect_x_posi_dd = 0;
	ctrl_data->x_posi_ctrl.expect_x_posi_d_old = 0;
	ctrl_data->x_posi_ctrl.s_x = 0;
	ctrl_data->y_posi_ctrl.expect_y_posi_d = 0;
	ctrl_data->y_posi_ctrl.expect_y_posi_old = 0;
	ctrl_data->y_posi_ctrl.expect_y_posi_dd = 0;
	ctrl_data->y_posi_ctrl.expect_y_posi_d_old = 0;
	ctrl_data->y_posi_ctrl.s_y = 0;
	...
	ctrl_data->x_speed_ctrl.out_pitch = 0.0;
	ctrl_data->y_speed_ctrl.out_roll = 0.0;
...
{
	/* 定义一个具有Ctrl_data_ty所有结构的指针body_ctrl */
	/* 定义一个具有x_posi_ctrl_ty所有结构的指针x_posi_ctrl，并把body_ctrl中x_posi_ctrl的值赋给它 */
	struct x_speed_ctrl_ty * x_speed_ctrl = &(body_ctrl->x_speed_ctrl);
	struct y_speed_ctrl_ty * y_speed_ctrl = &(body_ctrl->y_speed_ctrl);
	...
	if(body_ctrl->flight_status < FLIGHT_TAKING_OFF)
	{
		// x_posi_ctrl->x_posi_err_i = 0;
		// y_posi_ctrl->y_posi_err_i = 0;
		x_posi_ctrl->observe_out_pitch = 0;
		y_posi_ctrl->observe_out_roll = 0;
		return;
	}


	//期望位置赋初值
	// x_posi_ctrl->expect_x_posi = 0;
	// y_posi_ctrl->expect_y_posi = 0;

    //由RC通道计算xy速度控制量
	float exp_x_speed = remote_ctrl->pitch * 0.8;
	float exp_y_speed = -remote_ctrl->roll * 0.8;
	if ( fabs(exp_x_speed) < 0.01)    exp_x_speed = 0.0;
    if ( fabs(exp_y_speed) < 0.01)    exp_y_speed = 0.0;
	x_posi_ctrl->expect_x_posi += exp_x_speed * dt;
	y_posi_ctrl->expect_y_posi += exp_y_speed * dt;


	//计算位置误差(期望位置-实际位置)期望位置由动捕得到
	// x_posi_ctrl->x_posi_err = x_posi_ctrl->expect_x_posi - getOpFlowPosition(OP_X);
	// y_posi_ctrl->y_posi_err = y_posi_ctrl->expect_y_posi - getOpFlowPosition(OP_Y);
	x_posi_ctrl->x_posi_err = x_posi_ctrl->expect_x_posi - getBodyPosition(MC_X);
	y_posi_ctrl->y_posi_err = y_posi_ctrl->expect_y_posi - getBodyPosition(MC_Y);
    ...
    //dd_xd
    x_posi_ctrl->expect_x_posi_d = (x_posi_ctrl->expect_x_posi - x_posi_ctrl->expect_x_posi_old)/dt;
	y_posi_ctrl->expect_y_posi_d = (y_posi_ctrl->expect_y_posi - y_posi_ctrl->expect_y_posi_old)/dt;

	x_posi_ctrl->expect_x_posi_old = x_posi_ctrl->expect_x_posi;
	y_posi_ctrl->expect_y_posi_old = y_posi_ctrl->expect_y_posi;

 	x_posi_ctrl->expect_x_posi_dd = (x_posi_ctrl->expect_x_posi_d - x_posi_ctrl->expect_x_posi_d_old)/dt;
	y_posi_ctrl->expect_y_posi_dd = (y_posi_ctrl->expect_y_posi_d - y_posi_ctrl->expect_y_posi_d_old)/dt;

	x_posi_ctrl->expect_x_posi_d_old = x_posi_ctrl->expect_x_posi_d;
	y_posi_ctrl->expect_y_posi_d_old = y_posi_ctrl->expect_y_posi_d;
    ...
    //分数阶超螺旋滑模面S表达式
	double c1_x=100, c2_x=100,
		   c1_y=100, c2_y=100;	// static double s_x,s_y = 0;
    x_posi_ctrl->s_x = c1_x * x_err_lpf + c2_x*x_posi_ctrl->x_posi_err_d;
    y_posi_ctrl->s_y = c1_y * y_err_lpf + c2_y*y_posi_ctrl->y_posi_err_d;

	//滑模面一阶导中间变量
	double kx2 =0.5 ,ky2 = 0.5,lamda = -0.5;
	// double dot_z_x = -kx2 * pow(fabs(s_x) , 1 + 2 * lamda) * sign_s(s_x);
	// double dot_z_y = -ky2 * pow(fabs(s_y) , 1 + 2 * lamda) * sign_s(s_y);
	x_posi_ctrl->dot_z_x = kx2 * pow(fabs(x_posi_ctrl->s_x ) , 1 + 2 * lamda) * sign_s(x_posi_ctrl->s_x );
	y_posi_ctrl->dot_z_y = ky2 * pow(fabs(y_posi_ctrl->s_y ) , 1 + 2 * lamda) * sign_s(y_posi_ctrl->s_y );
	// LIMIT()

	//对中间变量求积分  存疑:是否需要赋初值
	x_posi_ctrl->z_x += x_posi_ctrl->dot_z_x * dt;
	y_posi_ctrl->z_y += y_posi_ctrl->dot_z_y * dt;

	//滑模面一阶导形式
	double kx1 = 1,ky1 = 1;
	x_posi_ctrl->u_x = kx1 * pow(fabs(x_posi_ctrl->s_x ) , 1 + lamda) * sign_s(x_posi_ctrl->s_x ) ;
	y_posi_ctrl->u_y = ky1 * pow(fabs(y_posi_ctrl->s_y ) , 1 + lamda) * sign_s(y_posi_ctrl->s_y ) ;

	//ESO
	x_posi_ctrl->err_ESO_x = getBodyPosition(MC_X)-x_posi_ctrl->x1;
    int beta=3 , m=0.05;
	x_posi_ctrl->x1 += dt*(x_posi_ctrl->x2+beta*beta*beta*x_posi_ctrl->err_ESO_x);
	x_posi_ctrl->x2 += dt*(x_posi_ctrl->x3+m*x_speed_ctrl->out_pitch+3*beta*beta*x_posi_ctrl->err_ESO_x);
	x_posi_ctrl->x3 += dt*(3*beta*x_posi_ctrl->err_ESO_x);

	y_posi_ctrl->err_ESO_y = getBodyPosition(MC_Y)-y_posi_ctrl->y1;
	y_posi_ctrl->y1 += dt*(y_posi_ctrl->y2+beta*beta*beta*y_posi_ctrl->err_ESO_y);
	y_posi_ctrl->y2 += dt*(y_posi_ctrl->y3+m*y_speed_ctrl->out_roll+3*beta*beta*y_posi_ctrl->err_ESO_y);
	y_posi_ctrl->y3 += dt*(3*beta*y_posi_ctrl->err_ESO_y);
	...
	x_speed_ctrl->out_pitch = 1/b * ( 1*x_posi_ctrl->expect_x_posi_dd+1*c1_x * x_posi_ctrl->x_posi_err_d  + x_posi_ctrl->u_x + x_posi_ctrl->z_x);// -1*x_posi_ctrl->x3);
	y_speed_ctrl->out_roll  = 1/b * ( 1*y_posi_ctrl->expect_y_posi_dd+1*c1_y * y_posi_ctrl->y_posi_err_d  + y_posi_ctrl->u_y + y_posi_ctrl->z_y);// -1*y_posi_ctrl->y3);
...
void SMC_xy_position_ctrl(struct ctrl_data_ty * body_ctrl, struct data_fusion_ty * body_data, struct remote_ctrl_ty * remote_ctrl, float dt)
{
	...
	double c = 2;
	double f = 1;
	x_posi_ctrl->s_x = c * x_posi_ctrl->x_posi_err + x_posi_ctrl->x_posi_err_d;
	y_posi_ctrl->s_y = c * y_posi_ctrl->y_posi_err + y_posi_ctrl->y_posi_err_d;
	
	x_speed_ctrl->out_pitch = (x_posi_ctrl->expect_x_posi_dd - c*x_posi_ctrl->x_posi_err - f * sign_s(x_posi_ctrl->s_x)) ;
	y_speed_ctrl->out_roll  = (y_posi_ctrl->expect_y_posi_dd - c*y_posi_ctrl->y_posi_err - f * sign_s(y_posi_ctrl->s_y)) ;
